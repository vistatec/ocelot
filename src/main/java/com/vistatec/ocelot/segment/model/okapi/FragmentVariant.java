...
/**
 * XLIFF 2.0 segment variant, implemented using the Okapi XLIFF 2.0 library
 * Fragment.
 */
public class FragmentVariant extends BaseSegmentVariant {
	private final static Logger LOG = LoggerFactory
			.getLogger(FragmentVariant.class);
	...
	private List<SegmentAtom> parseSegmentAtoms(Fragment frag) {
		...
		XLIFFFragmentRenderer fragmentRenderer = new XLIFFFragmentRenderer(
				frag, null);
	...
	public void updateSegmentAtoms(Segment okapiSegment) {
		this.segmentAtoms = parseSegmentAtoms(isTarget ? okapiSegment
				.getTarget() : okapiSegment.getSource());
	}
		...
		for (SegmentAtom atom : segmentAtoms) {
			if (atom instanceof TextAtom) {
				...
				copyAtoms.add(new TaggedCodeAtom(taggedCodeAtom.getTag(),
						taggedCodeAtom.getData(), taggedCodeAtom
								.getVerboseData()));
	...
	public FragmentVariant createCopy() {
		FragmentVariant copyFragment = new FragmentVariant(copyAtoms(),
				isTarget);
		return copyFragment;
	}
