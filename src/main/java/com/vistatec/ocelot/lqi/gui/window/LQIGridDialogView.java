...
/**
 * This dialog displays the LQI Grid. It lets users create new Language Quality
 * Issues for the selected segment.
 */
public class LQIGridDialogView extends JDialog implements Runnable, ActionListener {
	...
	public LQIGridDialogView(Window owner, LQIGridController controller, LQIGridConfigurations lqiGrid,
			boolean showManageConfBtn) {
	...
	private Component getCenterComponent() {
		...
		Action annotationAction = new AbstractAction() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int severityIndex = button.getSeverityColumn()
						- tableHelper.getLqiTableModel().getSeverityColsStartIndex();
				double severity = activeConfiguration.getSeverities().get(severityIndex).getScore();
				String severityName = activeConfiguration.getSeverities().get(severityIndex).getName();
		...
		tableContainer = new LQIGridTableContainer(activeConfiguration, tableHelper, annotationAction,
				LQIGridTableModel.ISSUES_ANNOTS_MODE);
